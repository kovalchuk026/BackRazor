    ...
    def forward(self, hidden_states):
        ...
        if self.prune_mode:
            if (not self.prune_after_softmax):
                attention_scores.masked_fill_(~self.attention_mask.detach(), float('-inf')) # '-inf' ?????? 
                attention_probs = self.softmax(attention_scores)                            #may be change masker to not use ~?

            if self.prune_after_softmax:
                # print("prune after SM")
                attention_probs_ = self.softmax(attention_scores)
                attention_probs = (~self.attention_mask.detach()).float() * attention_probs_  #opposite masking ??????? 
        else:
